// The 3D Die View panel: game-plan.md §4.2. Same `render.bin` bundle as the
// 2D Die View (`die-panel.ts`), extruded through the sky130 z-stack with
// three.js. Mirrors `die-panel.ts`'s structure: the panel exists and the
// workspace shell paints before `render.bin` arrives, the DOM is built
// lazily once the bundle resolves, and disposal tears down GL resources so
// switching away from the panel does not leak them.
//
// Falls back to a plain message -- never a crash, and never breaks the 2D
// panel, which has no dependency on this one -- if WebGL2 is unavailable or
...
export function die3dViewPanel(opts: Die3DPanelOptions): PanelDef {
  return {
    id: "die-view-3d",
    title: "Die View 3D",
      ...
      opts.bundleReady
        .then((bundle) => {
          ...
          try {
            ...
            loading.textContent =
              "3D unavailable — falls back to the 2D Die View. " +
