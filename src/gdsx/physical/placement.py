...
class Ordering:
    ...
    def __str__(self) -> str:
        ...
        direction = "increasing" if self.tau > 0 else "decreasing"
        return (
            f"{self.name}: {self.members} entries, tau={self.tau:+.3f} along {self.axis} "
            f"({direction}), {self.inversions}/{self.pairs} inverted -- {verdict}"
        )
